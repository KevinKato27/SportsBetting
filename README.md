...
- `config/master-prompt/Master_Sports_Betting_Prompt_v5.1_2026-09-05.txt` is the current consolidated constitution. It supersedes v5.0, v4.1, and every earlier prompt while preserving source-blind full-slate discovery and Paper Lab production controls.
- `config/prompt-stack.v5.1.json` points every research chat to that single canonical prompt. Earlier prompt and addendum files remain versioned history, not active instructions.
