...
import promptStack from '@/config/prompt-stack.v5.1.json';
    ...
    <Tabs value={tab} onValueChange={(value) => { setTab(value); setMenuOpen(false); }} className="app-shell">
      ...
      <main className="main-content">
        ...
        <TabsContent value="history">
          <div className="page-heading"><div><span className="kicker">REAL-MONEY RECORD</span><h1>Bet History</h1><p>Settled, prior-day, and advance-dated tickets stay here, separate from today’s card.</p></div><div className="gate-pill"><HistoryIcon size={15} /><span><b>THROUGH {historyThrough}</b></span></div></div>
        ...
        <TabsContent value="config">
          ...
          <div className="config-grid">
            <Panel title="Core constitution" icon={BookOpenCheck}><div className="config-card"><span>MASTER PROMPT</span><strong>v{promptStack.version} · {promptStack.effectiveDate}</strong><p>One consolidated constitution now governs OVERLAY. It preserves the source-blind full-slate audit and Paper Lab controls while adding evidence-cluster de-duplication, global slate ranking, counter-cases, selection-regret review, and a six-question pre-bet core.</p><div className="principles"><span>ANALYSIS FIRST</span><span>EDGE FIRST</span><span>PRICE FIRST</span><span>PORTFOLIO BEFORE PARLAY</span><span>PROMO LAST</span></div></div></Panel>
            <Panel title="Candidate discovery policy" icon={ShieldCheck}><div className="config-card"><span>EFFECTIVE PROMPT</span><strong>v{promptStack.version} · independent qualification</strong><p>GPT freezes a source-blind board across the {discoveryPolicy.discovery.scope.replaceAll('_', ' ')}, then evaluates supplied research. A parlay leg need not be a literal standalone single, but it must independently qualify as a strong, acceptably priced proposition before construction.</p><div className="principles"><span>SEARCH THE WHOLE SLATE</span><span>DE-DUPLICATE EVIDENCE</span><span>PAPER = R&amp;D</span><span>REAL = PRODUCTION</span><span>GRADES SUMMARIZE</span></div></div></Panel>
